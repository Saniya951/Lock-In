...
These `<input>` props are only relevant for uncontrolled inputs:
  ...
  * [`defaultChecked`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLInputElement#defaultChecked): A boolean. Specifies the initial value for `type="checkbox"` and `type="radio"` inputs.
  * [`defaultValue`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLInputElement#defaultValue): A string. Specifies the initial value for a text input.
...
An input like `<input />` is _uncontrolled._ Even if you pass an initial value like `<input defaultValue="Initial text" />`, your JSX only specifies the initial value. It does not control what the value should be right now.
...
As the error message suggests, if you only wanted to specify the _initial_ value, pass `defaultValue` instead:
[code] 
...
As the error message suggests, if you only wanted to specify the _initial_ value, pass `defaultChecked` instead:
[code] 
