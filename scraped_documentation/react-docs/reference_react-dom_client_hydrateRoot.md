...
After the root has finished hydrating, you can call `root.render` to update the root React component. **Unlike with[`createRoot`](/reference/react-dom/client/createRoot), you don’t usually need to do this because the initial content was already rendered as HTML.**
...
It is uncommon to call `root.render` on a hydrated root. Usually, you’ll [update state](/reference/react/useState) inside one of the components instead.
