...
const Chat = () => {
  ...
  // GitHub Sync States (Friend's work)
  const [currentSessionId, setCurrentSessionId] = useState(null);
  const [showSyncModal, setShowSyncModal] = useState(false);
  const [isSyncing, setIsSyncing] = useState(false);
  ...
  // Export Logic (Your work)
  const exportAsZip = async () => {
    try {
      if (Object.keys(sessionFiles).length === 0)
        return alert("No files to export");
      const zip = new JSZip();
      Object.entries(sessionFiles).forEach(([filename, content]) =>
        zip.file(filename, content),
      );
  ...
  // GitHub Sync Logic (Friend's work)
  const handleGithubSync = async (repoName) => {
    const token = localStorage.getItem("github_token");
    if (!token) return alert("Connect GitHub in Navbar first!");
    if (!currentSessionId) return alert("No active session found.");

    setIsSyncing(true);
    try {
      const res = await fetch(
        `http://localhost:8000/github/sync?session_id=${currentSessionId}&repo_name=${repoName}&token=${token}`,
        {
          method: "POST",
        },
      );
  ...
  // WebContainer Logic (Merged best version)
  const initializeWebContainer = async (files) => {
    try {
      if (webcontainerRef.current) webcontainerRef.current.teardown();
      const container = await WebContainer.boot();
      webcontainerRef.current = container;
      ...
      if (files["package.json"]) {
        ...
        await container.spawn("npm", ["run", "dev", "--", "--host", "0.0.0.0"]);
      }
  ...
  // Message Handling (Your Streaming Logic + Session ID tracking)
  const handleSendMessage = async (e) => {
    e.preventDefault();
    if (!input.trim()) return;
    ...
    try {
      const response = await fetch("http://localhost:8000/prompt/stream", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: userPrompt, search_method: false }),
      });
      ...
      while (true) {
        ...
        for (const message of parts) {
          ...
          const { type, data } = JSON.parse(message.replace("data: ", ""));

          if (type === "session_start") setCurrentSessionId(data.session_id);
          if (type === "status") {
            setMessages((prev) =>
              prev.map((m) =>
                m.id === botMessageId ? { ...m, text: data.message } : m,
              ),
            );
          }
          if (type === "file_created") {
            streamedFiles[data.filename] = data.content;
            setSessionFiles((prev) => ({
              ...prev,
              [data.filename]: data.content,
            }));
            if (!selectedFile) setSelectedFile(data.filename);
          }
          if (type === "complete") {
            if (data.preview_url) {
              setWebcontainerUrl(data.preview_url);
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === botMessageId ? { ...m, text: "❌ Error occurred." } : m,
        ),
      );
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      if (isDraggingRender) {
        const rightPanel = document.getElementById("right-panels");
        setRenderHeight(
          ((e.clientY - rightPanel.offsetTop) / rightPanel.clientHeight) * 100,
        );
      }
      if (isDraggingFileTree)
        setFileTreeWidth(
    ...
    return Object.entries(tree)
      ...
      .map(([name, item]) => (
        <div key={name} style={{ marginLeft: `${depth * 12}px` }}>
          {item.type === "folder" ? (
            <>
              <div className="flex items-center gap-1 text-xs font-semibold text-gray-400 py-1 px-2">
                <Folder size={12} /> {name}
              </div>
              {renderFileTree(item.children, depth + 1)}
            </>
    ...
    <div className="h-screen bg-[#050505] text-white flex flex-col">
      ...
      <div className="border-b border-white/5 bg-[#050505]/80 backdrop-blur-lg px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative" ref={menuRef}>
            <button
              onClick={() => setMenuOpen(!menuOpen)}
              className="w-10 h-10 rounded-lg hover:bg-white/10 flex items-center justify-center text-gray-400"
            >
              <MoreHorizontal className="w-5 h-5" />
            </button>
            {menuOpen && (
      ...
      <div className="flex-1 overflow-hidden flex" id="panels-container">
        ...
        >
          <div className="flex-1 overflow-y-auto px-4 py-6 space-y-4">
            {messages.map((m) => (
              ...
              >
                <div
                  className={`max-w-[85%] px-3 py-2 rounded-lg text-sm ${m.sender === "user" ? "bg-indigo-500/20 border border-indigo-500/50" : "bg-gray-800/50"}`}
                >
                  {m.text}
                </div>
              </div>
        ...
        <div
          ...
          className="w-1 hover:bg-indigo-500/50 cursor-col-resize"
        />
        ...
        >
          ...
          <div
            onMouseDown={() => setIsDraggingRender(true)}
            className="h-1 hover:bg-indigo-500/50 cursor-row-resize"
          />

          <div className="flex-1 flex overflow-hidden" id="code-panel">
            <div
              style={{ width: `${fileTreeWidth}px` }}
              className="border-r border-white/5 p-2 overflow-y-auto bg-[#0a0a0a]"
            >
              {renderFileTree(buildFileTree(sessionFiles))}
            </div>
            <div
            ...
            <div className="flex-1 bg-[#050505]">
              {selectedFile ? (
                <textarea
                  ...
                  className="w-full h-full bg-transparent p-4 font-mono text-xs outline-none resize-none text-gray-300"
                  spellCheck={false}
                />
              ) : (
