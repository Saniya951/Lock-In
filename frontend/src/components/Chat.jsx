...
const Chat = () => {
  ...
  // Add this useEffect to sync with Navbar's github_token if needed
  ...
  useEffect(() => {
    const handleStorageChange = (e) => {
      if (e.key === "github_token") {
        setIsGithubConnected(!!e.newValue);
      }
    };
  ...
  const promptProjectNameAndCreate = async () => {
    ...
    creatingProjectPromiseRef.current = (async () => {
      const enteredName = window.prompt(
        "Enter a project name to save this chat",
      );
      ...
      try {
        ...
        console.error(
          "[Projects] Failed to create project during first save:",
          error,
        );
        return null;
      }
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      const persistedProject =
        projectList.find((project) => project.id === storedProjectId) || null;
  ...
  const loadProjectFiles = async (projectId) => {
    ...
    try {
      ...
      if (Object.keys(fileMap).length > 0) {
        const hasPackageJson = Object.keys(fileMap).some((path) =>
          normalizeProjectPath(path).endsWith("package.json"),
        );
        if (hasPackageJson) {
  ...
  const handleStyleUpdate = async (styles) => {
    ...
    try {
      ...
      if (!elementInfo) {
        console.warn(
          "[Visual Editor] Element info not found in mapping:",
          dataId,
        );
        return;
      }
      ...
      const updatedContent = updateElementStyles(
        currentContent,
        dataId,
        styles,
      );
      ...
      setFiles((prev) => ({
        ...
        ...(containerSourceFile !== sourceFile
          ? { [containerSourceFile]: updatedContent }
          : {}),
      }));
      ...
      if (webcontainerRef.current && !sourceFile.includes(".py")) {
        try {
          ...
          console.log(
            "[Visual Editor] File updated in WebContainer:",
            containerSourceFile,
          );
        } catch (error) {
          console.warn(
            "[Visual Editor] Could not update in WebContainer:",
            error,
          );
        }
      }
  ...
  const handleGithubSync = async (repoName) => {
    ...
    if (!token) {
      alert("Please connect GitHub in the Navbar first!");
      setShowSyncModal(false);
      return;
    }
    // Use currentProjectId if currentSessionId is missing
    const identifier = currentSessionId || currentProjectId;
    if (!identifier) return alert("No active project or session found.");
  ...
  const initializeWebContainer = async (files) => {
    try {
      ...
      console.log(
        `[WebContainer] Resolved project root: ${projectRoot || "(root)"}`,
      );
      ...
      console.log(
        `[WebContainer] Step 1/4 complete in ${(performance.now() - bootStart).toFixed(0)} ms`,
      );

      // Write files to the container
      const mountStart = performance.now();
      console.log(
        `[WebContainer] Step 2/4: Mounting ${Object.keys(preparedFiles).length} files...`,
      );
      ...
      console.log(
        `[WebContainer] Step 2/4 complete in ${(performance.now() - mountStart).toFixed(0)} ms`,
      );
      const hasPackageJson = Boolean(
        preparedFiles[resolveProjectFile(projectRoot, "package.json")],
      );
      const hasPackageLockJson = Boolean(
        preparedFiles[resolveProjectFile(projectRoot, "package-lock.json")],
      );
      console.log(`[WebContainer] package.json present: ${hasPackageJson}`);
      console.log(
        `[WebContainer] package-lock.json present: ${hasPackageLockJson}`,
      );
      ...
      if (hasPackageJson) {
        ...
        console.log(
          `[WebContainer] Step 3/4 complete in ${(performance.now() - installStart).toFixed(0)} ms`,
        );
      }
      ...
      const serverReadyPromise = new Promise((resolve) => {
        container.on("server-ready", (port, url) => {
          console.log(
            `[WebContainer] Step 4/4 complete in ${(performance.now() - devStart).toFixed(0)} ms`,
          );
          console.log(
            `[WebContainer] Server is live at: ${url} on port ${port}`,
          );
          console.log(
            `[WebContainer] Total mount-to-render time: ${(performance.now() - totalStart).toFixed(0)} ms`,
          );
      ...
      if (
        ...
        preparedFiles[resolveProjectFile(projectRoot, "next.config.js")]
      ) {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () =>
              reject(new Error(`server-ready not fired within ${timeoutMs}ms`)),
            timeoutMs,
          ),
        ),
      ]);
  ...
  const injectInspectorScript = (htmlContent) => {
    if (
      ...
      htmlContent.includes(
        "[Inspector] ✓ SCRIPT LOADED IN IFRAME - Ready to detect clicks",
      )
    ) {
      return htmlContent;
    }
            ...
            switch (eventType) {
              case "session_start":
                ...
                if (currentThreadId) {
                  ...
                  setCurrentSessionId(currentThreadId);
                }
                setMessages((prev) =>
                  prev.map((msg) =>
                    msg.id === botMessageId
                      ? {
                          ...msg,
                          text: `Session started. Processing your request...`,
                        }
                      : msg,
                  ),
                );
                ...
                setMessages((prev) =>
                  prev.map((msg) =>
                    msg.id === botMessageId
                      ? {
                          ...msg,
                          text: `Planning complete. Tech stack: ${eventData.tech_stack}. Building files...`,
                        }
                      : msg,
                  ),
                );
              ...
              case "file_created": {
                ...
                console.log(
                  "[File Created] Event received - filename:",
                  filename,
                  "visualEditingEnabled:",
                  visualEditingEnabled,
                );
                ...
                if (filename.endsWith("index.html")) {
                  try {
                    ...
                    console.log(
                      "[Visual Editor] Inspector script ensured in index.html",
                    );
                  } catch (error) {
                    console.warn(
                      "[Visual Editor] Could not inject inspector into HTML:",
                      error.message,
                    );
                  }
                }
                ...
                if (shouldPreprocessJsxForInspector(filename, content)) {
                  try {
                    const { processedContent, mapping } = preprocessJsxWithIds(
                      content,
                      filename,
                    );
                    ...
                    console.warn(
                      "[Visual Editor] Could not preprocess",
                      filename,
                      ":",
                      error.message,
                    );
                ...
                setMessages((prev) =>
                  prev.map((msg) =>
                    msg.id === botMessageId
                      ? {
                          ...msg,
                          text: `Generated ${fileCount} files. Latest: ${filename}`,
                        }
                      : msg,
                  ),
                );
                  ...
                  prev.map((msg) =>
                    msg.id === botMessageId
                      ? {
                          ...
                          text:
                            finalExplanation ||
                            `✅ Project complete! Generated ${fileCount} files.`,
              ...
              case "complete": {
                ...
                if (
                  Object.keys(streamedFiles).length > 0 &&
                  streamedHasPackageJson
                ) {
                  // Use streamedFiles if it has fresh content from this turn
                  console.log(
                    "[WebContainer] No preview_url from backend. Initializing local WebContainer with new files...",
                  );
                  await initializeWebContainer(streamedFiles);
                } else if (
                  Object.keys(files).length > 0 &&
                  sessionHasPackageJson
                ) {
                  // Fallback: use saved files (useful for follow-up turns)
                  console.log(
                    "[WebContainer] Using cached files to reinitialize WebContainer...",
                  );
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      if (isDraggingRender) {
        ...
        if (rightPanel) {
          const newRenderHeight =
            ((e.clientY - rightPanel.offsetTop) / rightPanel.clientHeight) *
            100;
      ...
      if (isDraggingFileTree) {
        const newWidth =
          e.clientX - document.getElementById("code-panel").offsetLeft;
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      const saveCurrentFile = async () => {
        await persistFile(
          currentProjectId,
          normalizeProjectPath(activeFile),
          fileContent,
        );
      };
    ...
    const renderedFolders = folders.map((name) => {
      ...
      return (
        <div key={`folder-${name}`} style={{ marginLeft: `${depth * 12}px` }}>
          <div
            className={`text-xs font-semibold py-1 px-2 rounded transition-colors duration-300 ${
              isDarkMode
  ...
  const handleCodeEdit = (newCode) => {
    if (activeFile) {
      ...
      const containerSelectedFile = toContainerProjectPath(
      ...
      setFiles((prev) => ({
        ...
        ...(containerSelectedFile !== normalizedSelectedFile
          ? { [containerSelectedFile]: newCode }
          : {}),
      }));
      ...
      if (webcontainerRef.current && !normalizedSelectedFile.includes(".py")) {
        webcontainerRef.current.fs
          ...
          .catch((error) => {
            console.error(
              "[WebContainer] Failed to write edited file:",
              containerSelectedFile,
              error,
            );
          });
      }
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className={`border-b px-6 py-4 flex items-center justify-between backdrop-blur-lg transition-colors duration-300 ${
          isDarkMode
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={menuRef}>
            ...
            {menuOpen && (
              <div
                className={`absolute right-0 mt-2 w-48 rounded-lg shadow-lg z-50 transition-colors duration-300 ${
                  isDarkMode
      ...
      <div className="flex-1 overflow-hidden flex" id="workspace-shell">
        <aside
          className={`${isSidebarOpen ? "w-72" : "w-2"} relative overflow-visible border-r flex flex-col transition-all duration-300 ${
            isDarkMode
        ...
        >
            ...
            <div className="border-b border-white/5 p-4 space-y-3">
              {
                <>
                  ...
                  >
                    {loadingProjects
                      ? "Loading projects..."
                      : "First save asks for project name."}
                  </p>
                </>
              }
            </div>
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {currentProjectId === project.id && (
                          <span className="text-[10px] uppercase tracking-[0.2em] text-indigo-400">
                            Active
                          </span>
                        )}
                      </div>
          ...
          {isSidebarOpen && (
            <div
              className={`border-t p-4 text-xs ${isDarkMode ? "border-white/5 text-gray-500" : "border-gray-200 text-gray-600"}`}
            >
              {currentProjectId
        ...
        <div className="flex-1 overflow-hidden" id="panels-container">
          <div className="flex h-full">
            ...
            >
                ...
                {messages.length === 0 ? (
                  <div className="h-full flex items-center justify-center text-center">
                    <div className="space-y-3">
                      ...
                      <p
                        className={`text-xs max-w-xs transition-colors duration-300 ${
                          isDarkMode ? "text-gray-400" : "text-gray-600"
                        }`}
                      >
                        Send a message to get started with your coding
                        assistant.
                      </p>
                    </div>
                  </div>
              ...
              {/* Input Area */}
              <div
                className={`border-t backdrop-blur-lg px-4 py-3 transition-colors duration-300 ${
                  isDarkMode
                    ? "border-white/5 bg-[#050505]/80"
                    : "border-gray-200 bg-white/80"
                }`}
                ...
                <form onSubmit={handleSendMessage} className="flex gap-2">
                  ...
                  <div className="relative group">
                    <button
                      ...
                      title={
                        useDeepSearch
                          ? "Deep Search is ON (Tavily)."
                          : "Default Vector Search is ON."
                      }
                      aria-label={
                        useDeepSearch
                          ? "Deep search is on. Click to switch to default vector search"
                          : "Default vector search is on. Click to switch to deep Tavily search"
                      }
                    ...
                    <span
                      className={`pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md px-2 py-1 text-xs opacity-0 group-hover:opacity-100 transition-opacity ${
                        isDarkMode
                    ...
                    >
                      {useDeepSearch
                        ? "Mode: Deep Search (Tavily)"
                        : "Mode: Default Vector Search"}
                    </span>
                  </div>
                  <button
                    type="submit"
            ...
            >
              ...
              <div
                ...
                className={`flex flex-col border-b overflow-hidden transition-colors duration-300 ${
                  isDarkMode
              ...
              >
                <div className="flex h-full">
                  ...
                  <div
                    ...
                    className={`border-r overflow-y-auto flex-shrink-0 transition-colors duration-300 ${
                      isDarkMode
                        ? "bg-[#0a0a0a] border-white/5"
                        : "bg-gray-50 border-gray-200"
                    }`}
                  >
                    <div className="p-2">
                      <p
                        className={`text-xs font-semibold uppercase mb-2 transition-colors duration-300 ${
                          isDarkMode ? "text-gray-400" : "text-gray-600"
                        }`}
                      >
                        Files
                  ...
                  <div
                    ...
                    className={`w-1 cursor-col-resize transition-colors ${
                      isDarkMode
                        ? "bg-white/5 hover:bg-indigo-500/50"
                        : "bg-gray-200 hover:bg-indigo-400"
                    }`}
                  />

                  {/* Code Editor */}
                  <div
                    className={`flex-1 overflow-hidden flex flex-col transition-colors duration-300 ${
                  ...
                  >
                    {activeFile && files[activeFile] ? (
                      ...
                      <div className="flex items-center justify-center h-full">
                        ...
                        >
                          {currentProjectId
                            ? "Select a file to view and edit code"
