...
Create a virtual environment: This creates an isolated environment in a folder named venv to manage project-specific packages.
Bash

python -m venv venv

Activate the virtual environment: You must activate the environment every time you work on the project.
...
Install required packages: This command reads the requirements.txt file and installs all the necessary libraries into your venv.
Bash
...
Set up Environment Variables: The agent uses load_dotenv() which looks for a .env file in the current directory (which is now agent/).
...
Initialize the Vector Database: Run the setup script to populate the local Chroma vector database. This script will create an agent/chromadb/ directory (which is git-ignored) and fill it with data.
Bash
...
After completing the setup, you can run the main agent.
    ...
    Ensure you are still in the agent/ directory.
    ...
    The script will load the vector database and then prompt you to enter your project request.
...
After running this, commit the updated requirements.txt file to your Git repository.
